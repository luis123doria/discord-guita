...
export async function execute(interaction: CommandInteraction) {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
        { name: '✏️ Nombre', value: String(taskName || 'Sin nombre'), inline: true },
        ...
        { name: '⚡ Prioridad', value: String(prioridad || 'Sin prioridad'), inline: true },
        { name: '🏆 Puntos', value: `${puntos ?? 0}`, inline: true },
       { name: '👥 Asignada a:', value: usersInvolved.length > 0 ? usersInvolved.map(u => `• ${u.tag}`).join('\n') : 'Nadie', inline: true },
